...
def roi_pool(feature_map, rois, dim_reduce = 16):
	"""
	...
	with tf.variable_scope('roi_pool'):
		...
		ft_shape = tf.cast(tf.shape(feature_map), 'float32')
		im_dims_w = tf.ones_like(rois[:,0])*ft_shape[2]*dim_reduce
		im_dims_h = tf.ones_like(im_dims_w)*ft_shape[1]*dim_reduce
		...
		boxes           = rois[:,1:]
		normalization   = tf.cast( tf.stack( [im_dims_w, im_dims_h, im_dims_w, im_dims_h] , axis =1 ), dtype = tf.float32)
